...
def question2_4() -> str:
    ...
    """
    answer = "Gamma is a crucial factor in reinforcement learning that balances immediate and future rewards. Q-learning uses optimal value function estimates to update current estimates, and higher gamma values result in more accurate long-term value estimates. First-Visit Monte Carlo learns from complete episodes and actual rewards, so high gamma values have minor impact. Inappropriate gamma values can significantly affect Q-learning by overestimating or underestimating the true value function, thus affecting the bias-variance trade-off in Q-learning. Hence, gamma has more impact on Q-learning than on First-Visit Monte Carlo."  # TYPE YOUR ANSWER HERE (100 words max)
    return answer
...
def question3_6() -> str:
    ...
    return: answer (str): your answer as a string (100 words max)
    """
    answer = "A decay strategy that uses an exploration fraction parameter may be more suitable for different environments because it allows for more direct control of the exploration rate. On the other hand, the exponential decay strategy depends on the maximum number of timesteps, may reduce slowly, resulting in a higher exploration rate for longer than intended and may not even reach the minimum exploration rate desired. In contrast, the linear decay strategy allows for a predictable and gradual reduction in exploration rate with each episode, resulting in a smooth transition from exploration to exploitation."  # TYPE YOUR ANSWER HERE (100 words max)
    return answer
...
def question5_1() -> str:
    ...
    """
    answer = "5 configurations of the network hidden size - [64, 128], [128, 128], [256, 128], [512, 256], [512, 512] from Q4, provided the highest mean results. A log-uniform distribution ranging from 2e-4 to 3e-3 was applied for learning rates, ranging from 2e-4 to 3e-3. Higher learning rates converge faster but can be unstable, while lower learning rates are more stable but converge slower. Higher critic learning rates improves accuracy and stability while higher policy learning rates leads to quicker learning towards optimal policies. Tau was selected with a uniform distribution from 0.1 to 0.4, balancing the trade-off between more aggressive updates and potential instability/divergence. Batch size 128 is widely used in deep learning and was found to stabilise training. The gamma value of 0.99 was retained. A buffer capacity of 1e7 was picked, to store a large number of experiences for training. To promote smooth and gradual transitions and prevent abrupt changes during training, a cosine annealing schedule was employed to adjust the learning rates, tau, and noise standard deviation based on the current and maximum time-steps of the training loop. Hyper-parameters were selected using grid and random search based on their expected impact on convergence speed and performance."  # TYPE YOUR ANSWER HERE (200 words max)
    return answer